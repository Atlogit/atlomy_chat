...
class LexicalValueStorage:
    ...
    def store(self, lexical_value: LexicalValue) -> None:
        ...
        try:
            ...
            logger.info(f"Stored lexical value for lemma: {lexical_value.lemma}")
        except IOError as e:
            logger.error(f"Failed to store lexical value for lemma {lexical_value.lemma}: {str(e)}")
    ...
    def retrieve(self, lemma: str) -> Optional[LexicalValue]:
        ...
        if not os.path.exists(file_path):
            logger.warning(f"Lexical value for lemma {lemma} not found")
            return None
        try:
            ...
            logger.error(f"Failed to retrieve lexical value for lemma {lemma}: {str(e)}")
    ...
    def list_all(self) -> List[str]:
        try:
            ...
            logger.info(f"Listed {len(lemmas)} lexical values")
            return lemmas
        except OSError as e:
            logger.error(f"Failed to list lexical values: {str(e)}")
    ...
    def update(self, lexical_value: LexicalValue) -> None:
        try:
            ...
            logger.info(f"Updated lexical value for lemma: {lexical_value.lemma}")
        except LexicalValueStorageError as e:
            logger.error(f"Failed to update lexical value for lemma {lexical_value.lemma}: {str(e)}")
    ...
    def delete(self, lemma: str) -> bool:
        ...
        try:
            if os.path.exists(file_path):
                os.remove(file_path)
                logger.info(f"Deleted lexical value for lemma {lemma}")
                return True
            else:
                logger.warning(f"Lexical value for lemma {lemma} not found for deletion")
                return False
        except OSError as e:
            logger.error(f"Failed to delete lexical value for lemma {lemma}: {str(e)}")
    ...
    def get_version_history(self, lemma: str) -> List[LexicalValue]:
        ...
        if current_value:
            logger.info(f"Retrieved version history for lemma: {lemma}")
        else:
            logger.warning(f"No version history found for lemma: {lemma}")
